...
;(update ((add-to-counter 'dinu) 10) (list C1 C2 C3 C4) 3) 
...
(define (serve requests fast-counters slow-counters)
  ...
  (if (null? requests)
      ...
      (match (car requests)
       ...
       [(list name n-items)
        (serve (cdr requests) (update ((add-to-counter name) n-items) fast-counters (car (min-tt (check-n-items n-items))))
                                       (update ((add-to-counter name) n-items) slow-counters (car (min-tt (check-n-items n-items)))))]
       ...
       [(list 'remove-first)
        (if (null? (counters-with-customers (append fast-counters slow-counters)))
            (serve (cdr requests) fast-counters slow-counters)
            (serve (cdr requests) (update remove-first-from-counter fast-counters (car (min-et (counters-with-customers (append fast-counters slow-counters)))))
                   (update remove-first-from-counter slow-counters (car (min-et (counters-with-customers (append fast-counters slow-counters)))))))]
