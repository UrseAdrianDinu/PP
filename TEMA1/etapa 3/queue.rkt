...
; Veți întoarce coada actualizată.
(define (dequeue q)
  (if (null? (queue-left q)) (make-queue (cdr (reverse (queue-right q))) '() (- (queue-size-r q) 1) 0)
...
(define (top q)
  (if (null? (queue-left q)) (last (queue-right q)) (car (queue-left q))))
