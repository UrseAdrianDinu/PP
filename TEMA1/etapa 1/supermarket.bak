...
(define (serve requests C1 C2 C3 C4)
  ...
  (define (check-n-items n-items)
    (if (<= n-items ITEMS) (list C1 C2 C3 C4) (list C2 C3 C4)))
  ...
  (if (null? requests)
      ...
      (match (car requests)
        ...
        [(list name n-items)        (cond
                                      (( = (car (min-tt (check-n-items n-items))) 1) (serve  (cdr requests) (add-to-counter C1 name n-items) C2 C3 C4))
                                      (( = (car (min-tt (check-n-items n-items))) 2) (serve  (cdr requests) C1 (add-to-counter C2 name n-items) C3 C4))
                                      (( = (car (min-tt (check-n-items n-items))) 3) (serve  (cdr requests) C1 C2 (add-to-counter C3 name n-items) C4))
                                      (( = (car (min-tt (check-n-items n-items))) 4) (serve  (cdr requests) C1 C2 C3 (add-to-counter C4 name n-items))))])))
