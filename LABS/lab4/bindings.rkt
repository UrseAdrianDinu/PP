...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ([segment (get-line-segment)]
    ...
    (sqrt (+ (* (- (car start) (car end)) (- (car start) (car end))) (* (- (cdr start) (cdr end)) (- (cdr start) (cdr end))))))) 
...
(define (num-concat x y)
  (let ((+ (lambda (x y) (string->number (string-append (number->string x) (number->string y))))))
