...
(define (num->base n b)
  (if (= 0 n) '() (append (num->base (quotient n b) b) (list (modulo n b)))))
...
(define (rev L)
  (if (null? L) null (append(rev(cdr L)) (list(car L )))))
...
(define (palindrome? L)
(if (null? L) #t (equal? (deep-rev L) L)))
...
(define (palindrome-list? L)
  (if (null? L) #t (equal? (rev L) (deep-rev L))))
