...
insertElem (BSTNode value left right) elem
  | value == elem = BSTNode value left right
  | value < elem = BSTNode value left (insertElem right elem)
  | value > elem = BSTNode value (insertElem left elem) right

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil a = Nothing
findElem (BSTNode value left right) elem
...
mapTree f (BSTNode value left right) = BSTNode (f value) (mapTree f left)  (mapTree f right)

foldlTree :: (b -> a -> b) -> b -> BST a -> b
foldlTree f acc BSTNil = acc
foldlTree f acc (BSTNode value left right) =
  foldlTree f (foldlTree f (f acc value) left) right
...
consElem a (Elem b) = List [Elem a, Elem b]
consElem a (List b) = List (Elem a : b)

consList :: SList a -> SList a -> SList a
consList a (Elem b)  = List [a, Elem b]
...
-}
newtype DList a = DL ([a] -> [a]) -- de rafinat tipul argumentului lui DL
