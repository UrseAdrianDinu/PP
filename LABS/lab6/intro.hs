...
numToBase n b = if n==0 then [] else numToBase (n `quot` b) b ++ [n `mod` b]

-- pattern matching
numToBase2 :: Integer -> Integer -> [Integer]
numToBase2 0 _ = []
numToBase2 n b = numToBase (n `quot` b) b ++ [n `mod` b]

-- gărzi
numToBase3 :: Integer -> Integer -> [Integer]
numToBase3 n b
    | n==0 = []
    | otherwise =  numToBase3 (n `quot` b) b ++ [n `mod` b]

-- case of
numToBase4 :: Integer -> Integer -> [Integer]
numToBase4 n b = case n of
  0 -> []
  _ ->  numToBase4 (n `quot` b) b ++ [n `mod` b]
...
removeDuplicatesLeft l =  reverse (foldl (\acc x -> if x `elem` acc then acc else x:acc) [] l)
...
removeDuplicatesRight l = foldr (\x acc -> if x `elem` acc then acc else x:acc) [] l
...
findStringsLongerThanTenChars l = filter (\c -> length c >= 10) l
...
buildPairsStringLength l = map (\c -> (c,length c)) l

buildPairsStringLength2 :: [String] -> [(String, Int)]
buildPairsStringLength2 l = [(c,length c) | c<-l]
